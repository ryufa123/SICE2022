...
def read_csv(csvPath, train_number):
    ...
    feature = data_df[['F0', 'F1', 'F2', 'F3','F4','F5','F6','F7','F8','F9','F10','F11','F12','F13','F14','F15','F16',
                       ...
                       'F52','F53','F54','F55','F56','F57','F58','F59','F60','F61','F62','F63','F64','F65']].values
...
if __name__ == "__main__":
    ...
    s = []
    for i in range(1):
        ##Labels of different categories can be directly modified here(three-class:012 2-class:100/010/001)
        classNameDict = {'N': 0 , 'P': 1 }
        print("==============================")
        ##The first 75 are used for training and the last 75 are used for testing
        trainFeature, trainlabel, valFeature, vallabel = read_csv(r'C:\Users\Mechrevo\Desktop\autumn\SICE2022\NP.csv',48)
        a = np.array(trainFeature); b = np.array(valFeature)
        x = np.r_[a,b]
        a = np.array(trainlabel); b = np.array(vallabel)
        y = np.r_[a,b]
        print(y)
        kfolder = KFold(n_splits=49,random_state=1)
        for train, test in kfolder.split(x,y):
            lda = LinearDiscrtAnays()                                   
            lda.train(x[train],y[train])
            pre = lda.predict(x[test])
            s.append(sum(pre==y[test])/x[test].shape[0])
            print(s)
            #print("Correct rate：{:.3f}".format(sum(pre==y[test])/x[test].shape[0]))
        i=i+1
    #print(a)
    print("Mean is :{:.4}".format(np.mean(s))) 
    print("==============================")
    print("Variance is :{:.3}".format(np.var(s)))
